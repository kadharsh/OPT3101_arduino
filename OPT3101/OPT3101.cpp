...
void OPT3101::resetAndWait()
{
  ...
  _i2cPort->write(0);
  _i2cPort->write(1);
  lastError = _i2cPort->endTransmission();
  if (lastError) { return; }
...
void OPT3101::writeReg(uint8_t reg, uint32_t value)
{
  _i2cPort->beginTransmission(address);
  _i2cPort->write(reg);
  _i2cPort->write((uint8_t)(value >> 0));
  _i2cPort->write((uint8_t)(value >> 8));
  _i2cPort->write((uint8_t)(value >> 16));
  lastError = _i2cPort->endTransmission();
}
...
uint32_t OPT3101::readReg(uint8_t reg)
{
  _i2cPort->beginTransmission(address);
  _i2cPort->write(reg);
  lastError = _i2cPort->endTransmission(false); // no stop (repeated start)
  if (lastError)
  {
    return 0;
  }

  uint8_t byteCount = _i2cPort->requestFrom(address, (uint8_t)3);
  if (byteCount != 3)
  {
    lastError = 50;
    return 0;
  ...
  lastError = 0;
  uint32_t value = _i2cPort->read();
  value |= (uint16_t)_i2cPort->read() << 8;
  value |= (uint32_t)_i2cPort->read() << 16;
  return value;
}

void OPT3101::init(TwoWire &wirePort)
{
  _i2cPort = &wirePort;
  resetAndWait();
  if (getLastError()) { return; }
